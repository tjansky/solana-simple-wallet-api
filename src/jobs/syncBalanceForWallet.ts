...
export const syncBalanceForWallet = async (address: string) => {
  ...
  const existingSol = await prisma.tokenBalance.findFirst({
    where: { walletId: address, mint: "SOL" },
  });
  ...
  console.error(`Error fetching SPL tokens for ${address}:`, err.message);
}
